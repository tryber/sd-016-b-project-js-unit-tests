...
// Contribuição: Gabriel Silvestre 
// https://github.com/tryber/sd-016-b-project-js-unit-tests/pull/20/commits/84bce5f68d933f029c0fc9990a43f8ae8eeaae6c
// Mais explicação no zoom: 01/10/2021

let restaurante = {};

const adicionarConsumo = (strRecebe) => {
  restaurante.consumption.push(strRecebe);
};

const percorreTodosItens = () => {
  const comidas = Object.entries(restaurante.fetchMenu())[0][1];
  const bebidas = Object.entries(restaurante.fetchMenu())[1][1];

  let soma = 0;

  restaurante.consumption.forEach((item) => {
    if (comidas[item] !== undefined) soma += comidas[item];
    if (bebidas[item] !== undefined) soma += bebidas[item];
  });
  return soma + (soma * 0.1);
};
// Contribuição: Gabriel Pinheiro 
// Auxílio no zoom: 02/10/2021 14:23
const createMenu = (obj) => {
  restaurante = { fetchMenu: () => obj,
  consumption: [],
  order: adicionarConsumo, // excuta a função 
  pay: percorreTodosItens,
};
...
return restaurante;
  ...
  // console.log(objetoRetornado.pay());
    // console.log(`${Object.values(createMenu(objeto.fetchMenu))}`); // retorno   () => obj
    //------------------------------------------------------------------------------------------
    //  PASSO 2: Adicione ao objeto retornado por `createMenu` uma chave `consumption` que, como valor inicial, tem um array vazio.
    // console.log(`${Object.values(createMenu(objeto.fetchMenu))}`);
