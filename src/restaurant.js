...
const createMenu = (myMenu) => {
  const newMenu = {
    ...
    pay: () => {
      let totalCost = 0;
      for (let item of newMenu.consumption) {
        if (Object.keys(myMenu.food).includes(item)) {
          totalCost += myMenu.food[item];
        } else if (Object.keys(myMenu.drink).includes(item)) {
          totalCost += myMenu.drink[item];
        }
      }
      return totalCost;
    },
