...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  it('Verifica se ao receber um raio, a função `circle` retorna um objeto contedos os valores esperados', () => {
    ...
    const radius = 3;
    const area = circle(radius).area;
    const circumference = circle(radius).circumference;
    expect(area).toBeCloseTo(radius * 3 * 3.14);
    expect(circumference).toBeCloseTo(radius * 3.14 * 2);
  });
});
