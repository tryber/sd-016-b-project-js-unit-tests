...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    expect(Object.keys(menu2.fetchMenu())).toEqual(['food', 'drink']);
    ...
    const menu3 = createMenu({food: 'burguer', drink: 'cola'});

    expect(menu3.fetchMenu()).toEqual({food: 'burguer', drink: 'cola'});

    // TESTE 4: Verifique se 'objetoRetornado.consumption', após a criação do menu, retorna um array vazio.
    const menu4 = createMenu({food: 'none'});

    expect(menu4.consumption).toEqual([]);
    ...
    const menu5 = createMenu({foods: '', drinks: ''});

    menu5.order('coxinha');
    expect(menu5.consumption).toContain('coxinha');
    ...
    menu6.order('sopa');
    menu6.order('sashimi');
    expect(menu6.consumption).toEqual(['coxinha', 'agua', 'sopa', 'sashimi']);

    // TESTE 7: Verifique se a função `order` aceita que pedidos repetidos sejam acrescidos a consumption.
    const menu7 = createMenu({});
    
    menu7.order('coxinha');
    menu7.order('agua');
    menu7.order('coxinha');
    expect(menu7.consumption).toEqual(['coxinha', 'agua', 'coxinha']);
    ...
    menu8.order('agua');
    menu8.order('coxinha');
    expect(menu8.pay()).toBeCloseTo(13.20);
  });
});
