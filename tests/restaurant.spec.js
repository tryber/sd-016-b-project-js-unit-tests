...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  ...
  it('Requisito 1- Verifica se a função `createMenu` tem o comportamento esperado', () => {
  ...
  it('Requisito 2- Verifique se objetoRetornado.fetchMenu() retorna um objeto cujas chaves são somente `food` e `drink`', () => {
    /*const objetoRetornado = createMenu({
      food: {},
      drink: {}
    });*/
  ...
  it('Requisito 3- Verifique se o menu passado pra função createMenu é identico ao menu recuperado pela função objetoRetornado.fetchMenu', () => {
    expect(newObj.fetchMenu()).toEqual({
      food: { 1: 'hamburguer' },
      drink: { 1: 'coca' }
    });
  });
  ...
  it('Requisito 4- Verifique se objetoRetornado.consumption, após a criação do menu, retorna um array vazio.', () => {
  ...
  it('Requisito 6- Verifique se, ao adicionar três pedidos, dentre bebidas e comidas, o array `objetoRetornado.consumption` contém os itens pedidos.', () => {
    objetoRetornado.order("sashimi");
    objetoRetornado.order("sopa");
    objetoRetornado.order("agua");
    expect(objetoRetornado.consumption).toEqual(['coxinha', 'sashimi', 'sopa', 'agua']);
  });
  // ```
  // objetoRetornado.order("coxinha");
  // objetoRetornado.order("agua");
  ...
  it('Requisito 7- Verifique se a função `order` aceita que pedidos repetidos sejam acrescidos a consumption.', () => {
    objetoRetornado.order('coxinha');
    objetoRetornado.order('agua');
    objetoRetornado.order('coxinha');
    expect(objetoRetornado.consumption).toEqual(['coxinha', 'sashimi', 'sopa', 'agua', 'coxinha', 'agua', 'coxinha']);
  })
  // ```
  // objetoRetornado.order('coxinha');
  // objetoRetornado.order('agua');
