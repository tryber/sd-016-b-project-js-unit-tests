...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifique se o retorno da função createMenu() é um objeto que possui a chave fetchMenu com uma função.', () => {
    expect(typeof createMenu().fetchMenu).toBe('function')
  });

  it('Verifique se `objetoRetornado.fetchMenu()` retorna um objeto com chaves `food` e `drink` considerando o parametro passado em  creatMenu', () => {
  ...
  it('Verifique se a função associada à chave `order` no objeto,passando uma string como parâmetro, tal string é adicionada ao array retornado em objetoRetornado.consumption' , () => {
  ...
  it('Verifique se, ao adicionar três pedidos, o array `objetoRetornado.consumption` contém os itens pedidos.', () => {
    const restaurante = createMenu({ food: { coxinha: 3.9, sopa: 9.9 }, drink: { agua: 3.9, cerveja: 6.9 } });
    restaurante.order('coxinha');
  ...
  it('Verifique se,`objetoRetornado.pay()`, retorna-se a soma dos preços dos pedidos em`objetoRetornado.consumption`', () => {
    const restaurante = createMenu({ food: { coxinha: 3.9, sopa: 9.9 }, drink: { agua: 3.9, cerveja: 6.9 } });
    restaurante.order('coxinha');
    restaurante.order('agua');
    restaurante.order('coxinha');
