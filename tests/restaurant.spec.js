...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    const testObj = { test: {}, tobeSure: 'yes' }
    ...
    // ```
    expect(createMenu({ test: {}, tobeSure: 'yes' }).fetchMenu()).toEqual({ test: {}, tobeSure: 'yes' })
    ...
    // ```
    expect(createMenu({ test: {}, tobeSure: 'yes' }).consumption).toEqual([])
    ...
    // ```
    let test8 = createMenu({ food: {'coxinha': 3.50, 'sopa': 9.00}, drink: {'agua': 3.00, 'cerveja': 6.00} })
    test8.order('coxinha')
    test8.order('agua')
    test8.order('coxinha')
    expect(test8.pay()).toEqual((10 * 0.1) + 10)
