...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    const objetoRetornado = createMenu(itemObj); // Retorno: { fetchMenu: () => {}, ... }
    ...
    expect(Object.keys(objetoRetornado.fetchMenu())).toContain('drink', 'food');
    // ```
    ...
    expect(objetoRetornado.fetchMenu()).toEqual(itemObj);
    // ```
    ...
    // REVIEW - Ao inserir o item na ordem de serviço, a função retorna um número, e utilizo o método 'expect.any(Number)' para recuperar esse retorno e passar pelo expect principal. Após isso, verifico se contém o item inserido em 'objetoRetornado.consumption'.

    const itemConsumption = 'coxinha';
    expect(objetoRetornado.order(itemConsumption)).toEqual(expect.any(Number));
    expect(objetoRetornado.consumption).toContain(itemConsumption);
    // ```
    ...
    // REVIEW - Utiliza o matcher toEqual, com o método arrayContaining para encontrar recorrências no 'objetoRetornado.consumption'.

    expect(objetoRetornado.consumption).toEqual(expect.arrayContaining(['coxinha', 'agua', 'sopa', 'sashimi']));
    ...
    expect(objetoRetornado.consumption).toEqual(expect.arrayContaining(['coxinha', 'agua', 'coxinha']));
    ...
    // NOTE - Apenas seguindo lógica de somar os valores de todos os items de 'objetoRetornado.consumption' que utilizei no arquivo principal.
    ...
    for (let i = 0; i < objetoRetornado.consumption.length; i += 1) {
      for (let j = 0; j < objetoRetornado.consumption.length; j += 1) {
        if (objetoRetornado.consumption[i] === arrDrink[j]) {
          somaDosPrecosDosPedidos += itemObj.drink[arrDrink[j]];
        } else if (objetoRetornado.consumption[i] === arrFood[j]) {
          somaDosPrecosDosPedidos += itemObj.food[arrFood[j]];
        }
      }
    }
    somaDosPrecosDosPedidos += (somaDosPrecosDosPedidos * 0.1);

    expect(objetoRetornado.pay()).toEqual(somaDosPrecosDosPedidos);
