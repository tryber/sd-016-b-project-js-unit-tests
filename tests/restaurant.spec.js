...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  ...
  it('Verifica se o menu passado pra função createMenu é identico ao menu recuperado pela função "objetoRetornado.fetchMenu"', () => {
    const objetoRetornado = createMenu({ food: 'carne', drink:'sprite'})
    expect(objetoRetornado.fetchMenu()).toEqual({ food: 'carne', drink:'sprite'});
  })
  ...
  it('Verifica se ao acionar a chave "order", é adicionado a string a chave consumption', () => {
    const objetoRetornado = createMenu({ food: 'coxinha', drink:'sprite'});
    objetoRetornado.order("coxinha");
  ...
  it('Verifica se ao chamar `objetoRetornado.pay()`, retorna-se a soma dos preços de tudo que foi pedido.', () => {
    const objetoRetornado = createMenu({
      food: {'coxinha': 3.90, 'sanduiche': 9.90},
      drinks: {'agua': 3.90, 'cerveja': 6.90}
    });
    ...
    objetoRetornado.order("agua");
    objetoRetornado.order("coxinha");
    expect(objetoRetornado.pay()).toBeCloseTo(11.7*1.1, 0.00001);
  })
