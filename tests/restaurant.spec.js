...
// Colega Israel Sant'Ana ajudou no entendimento e lógica dos itens 1, 2 e 3.
// Colega Hugo Daniel, deu aula sobre o requisito, explicando passo a passo da questão (Itens 01 ao 09).
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Teste 1.1- Verifique se o retorno da função createMenu() é um objeto que possui a chave fetchMenu.' , () => {
   expect(createMenu()).toHaveProperty('fetchMenu');
  });
  it('Teste 1.2- Verifique se o retorno da função createMenu() é uma qual tem como valor uma função.' , () => {
  ...
  it('Teste 3 - Verifique se o menu passado pra função createMenu é identico ao menu recuperado pela função "objetoRetornado.fetchMenu".' , () => {
    const meuRestaurante = createMenu({ food: { 'coxinha': 3.9, 'sopa': 9.9 }, drink: { 'agua': 3.9, 'cerveja': 6.9 } });
    expect(meuRestaurante.fetchMenu()).toMatchObject({ food: { coxinha: 3.9, sopa: 9.9 }, drink: { agua: 3.9, cerveja: 6.9 }});
  });
  ...
  it('Teste 8 - Verifique se, ao chamar "objetoRetornado.pay()", retorna-se a soma dos preços de tudo que foi pedido, conforme registrado em "objetoRetornado.consumption".' , () => {
    ...
    meuRestaurante.order('agua');
    meuRestaurante.order('coxinha');  
    expect(meuRestaurante.pay()).toEqual(11.7);
  });
});
