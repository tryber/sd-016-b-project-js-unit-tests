...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  ...
  it('Verifique se a chave fetchMenu tem como valor uma funçao', () => {
  ...
  it('Verifique se objetoRetornado.fetcjMenu, retorna um objeto cujas chaves são somente `food` e `drink`', () => {
  ...
  it('Verifique se o menu passado pra função createMenu é identico ao menu recuperado pela função objetoRetornado.fetchMenu', () => {
    const objetTest = {};
    expect(createMenu(objetTest).fetchMenu()).toEqual(objetTest);
  });
  // ```
  ...
  // ```
  it('Verifique a função associada a chave order no obj retornado', () => {
    const objetoRetornado = createMenu({});
  ...
  it('Verificar ao adicionar 3 pedidos, dentre bebidas e comidas, o array `objetoRetornado.consumption` contém os itens pedidos', () => {
    const objetoRetornado = createMenu({
      food: {},
      drink: {}
    });
...
// Auxílio no zoom: 02/10/2021 14:23
  ...
  it('verifique se, ao camar objetoRetornado pay se retorna precos de tudo que foi pedido ', () => {
