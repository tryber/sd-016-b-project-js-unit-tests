...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // ```
    expect(menu === createMenu(menu).fetchMenu()).toBeTruthy();
    ...
    // ```
    const newOrder = createMenu(menu);
    newOrder.order('coxinha');
    newOrder.order('agua');
    newOrder.order('coxinha');
    expect(newOrder.consumption).toEqual(['coxinha', 'agua', 'coxinha']);
    ...
    const orderRequisito10 = createMenu(menu);
    orderRequisito10.order('coxinha');
    orderRequisito10.order('agua');
    orderRequisito10.order('coxinha');
    expect(orderRequisito10.pay()).toBeCloseTo(11.7);
  });
});
